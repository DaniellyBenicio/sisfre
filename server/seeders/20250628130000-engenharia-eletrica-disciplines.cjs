...
module.exports = {
  async up(queryInterface, Sequelize) {
    const engenhariaEletricaCourseId = 3; 
    const courseDisciplinesToInsert = [];
    const csvCombinations = new Set();
    const dbCombinations = new Set();
    const csvFilePath = "../server/data/disciplines-eng-eletrica.csv";
    let fileContent;
    try {
  ...
  async down(queryInterface, Sequelize) {
    ...
    console.log(
      `Registros para o curso Engenharia Elétrica removidos com sucesso.`
    );
  },
};
