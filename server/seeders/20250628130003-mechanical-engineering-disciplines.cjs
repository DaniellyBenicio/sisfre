...
module.exports = {
  async up(queryInterface, Sequelize) {
    const engenhariaEletricaCourseId = 4;
    const courseDisciplinesToInsert = [];
    const csvCombinations = new Set();
    const dbCombinations = new Set();
    const csvFilePath = "../server/data/disciplines-mechanical-engineering.csv";
    let fileContent;
    try {
  ...
  async down(queryInterface, Sequelize) {
    ...
    console.log(
      `Registros para o curso Engenharia Mecânica removidos com sucesso.`
    );
  },
};
