...
const UserEditDialog = ({ open, onClose, userToEdit, onSubmitSuccess }) => {
  ...
  const formik = useFormik({
    ...
    enableReinitialize: true, // Reinitialize form when userToEdit changes
    onSubmit: async (values) => {
      ...
      try {
        ...
        const updatedUser = {
          id: response.data.id || response.data.user?.id || userToEdit.id,
          username: values.username,
          email: values.email,
          accessType: values.accessType,
        };
        ...
        const errorMessage =
          err.response?.data?.error || `Erro ao atualizar usuário: ${err.message}`;
        setError(
